...
const state = {
    ...
    async exec(message, [transaction_id]) {
        ...
        if (!isMessage(proceedingMessage))
            return;
        //Checks
        if (isNaN(+message.text) || message.text.includes("-"))
            return await bot.editMessageText(text.t("errors.is_not_number"), {
        ...
        if (message.text.includes("e") || (message.text.split(".")[1] ? (message.text.split(".")[1].length > 2) : false))
            return await bot.editMessageText(text.t("errors.too_long_float"), {
                chat_id: proceedingMessage.chat.id,
                message_id: proceedingMessage.message_id,
                parse_mode: "MarkdownV2"
            })
        ...
        if (+message.text === 0)
            return await bot.editMessageText(text.t("errors.is_null"), {
                chat_id: proceedingMessage.chat.id,
                message_id: proceedingMessage.message_id,
                parse_mode: "MarkdownV2"
            })
        ...
        if (!transaction)
            throw new Error("Trasaction wasn't found!!! ID = ", transaction_id);
