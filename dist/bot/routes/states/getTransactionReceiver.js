...
const state = {
    ...
    async exec(message) {
        ...
        if (message.text.includes("e") || message.text.includes("."))
            return await bot.editMessageText(text.t("errors.is_not_ceil"), {
                chat_id: proceedingMessage.chat.id,
                message_id: proceedingMessage.message_id,
                parse_mode: "MarkdownV2"
            })
                .catch(err => console.log("❌ Error | Error message (not ceil) wasn't sent! Reason = ", err));
        if (message.text.length < 5 || message.text.length > 20)
            return await bot.editMessageText(text.t("errors.wrong_length"), {
                chat_id: proceedingMessage.chat.id,
                message_id: proceedingMessage.message_id,
                parse_mode: "MarkdownV2"
            })
