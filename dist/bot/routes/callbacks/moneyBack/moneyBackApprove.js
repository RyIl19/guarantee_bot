...
const callback = {
    ...
    async exec(query, [transaction_id]) {
        if (!query.message || !query.message.from)
            return;
        text.setDefaultNamespace("transactions");
        //Get transaction
        const transaction = transactions.get(transaction_id);
        if (!transaction)
            return console.log(`❌ Error | Transaction wasn't found! Transaction ID = ${transaction_id}`);
        if (transaction.processing_status)
            return console.log("❌ Error | Transaction is already processing!");
