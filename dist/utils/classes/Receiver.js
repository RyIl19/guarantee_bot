...
class Receiver {
     ...
     */
    static async inform(transaction_id, callback_query_id) {
        //Get transaction
        const transaction = transactions.get(transaction_id);
        if (!transaction)
            throw new Error(`Error | Transaction wasn't found! Transaction ID = ${transaction_id}`);
        //Setup text
    ...
    static async agree(transaction_id, query) {
        if (!query.message)
            throw new Error("❌ Error | Callback query has no message included!");
        text.setDefaultNamespace("receiver");
        //Get transaction
        const transaction = transactions.get(transaction_id);
        if (!transaction)
            throw new Error("❌ Error | Transaction wasn't found!");
        //Inform receiver
    ...
    static async disagree(transaction_id, query) {
        if (!query.message)
            throw new Error("❌ Error | Callback query has no message included!");
        text.setDefaultNamespace("receiver");
        //Get transaction
     ...
     * @returns
     */
    static async requestMoneyBack(transaction_id) {
        text.setDefaultNamespace("receiver");
        //Get transaction
        const transaction = transactions.get(transaction_id);
        if (!transaction)
